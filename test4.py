...
class BertRegressionModel(nn.Module):
    def __init__(self):
        ...
        self.hyponet = HypoMlp(depth=3, in_dim=3, out_dim=2, hidden_dim=4)
        total_params = 0
        for name, shape in self.hyponet.param_shapes.items():
            total_params += shape[0] * shape[1]
        print('total hyponet params:', total_params)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        print('BERT outputs shape:', outputs.shape)
        print('hyponet params shape:', self.hyponet.param_shapes)
        params = dict()
        start_idx = 0
        for name, shape in self.hyponet.param_shapes.items():
