...
try:
    ...
    print(f"Import failed: lightgbm")
    lgb = None
try:
    import xgboost as xgb
except Exception:
    print(f"Import failed: xgboost")
    xgb = None
try:
    from tabpfn import TabPFNClassifier
except Exception:
    print(f"Import failed: tabpfn")
...
class TorchMLP(nn.Module):
    def __init__(self, input_dim: int, n_layers: int, width: int, output_dim: int):
        super().__init__()
        layers = []
        in_dim = input_dim
...
def train_torch_mlp(X_train, y_train, X_val, y_val, mlp_config, num_classes, device='cpu', epochs=30, batch_size=32, lr=1e-3, seed=0, ckpt_path=None):
    if torch is None:
        raise RuntimeError("PyTorch not installed. Install torch to train the MLP.")
    set_global_seed(seed)
    model = TorchMLP(input_dim=X_train.shape[1], n_layers=mlp_config[0], width=mlp_config[1], output_dim=num_classes).to(device)
...
def make_cv(y, n_folds, seed):
    ...
    if n_splits < 2:
        n_splits = 2  # but if n_samples <2, will error, but assume not
    return KFold(n_splits=n_splits, shuffle=True, random_state=seed)


def cross_val_accuracy(model_name, X, y, seed, num_classes, mlp_arch=None, mlp_epochs=30):
    """Run CV for given model, return mean accuracy."""
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        if model_name == "logistic":
            clf = LogisticRegression(max_iter=1000, random_state=seed, multi_class='ovr')
            ...
            params = {"objective": "multiclass", "metric": "multi_logloss", "num_class": num_classes, "verbosity": -1, "seed": seed}
            ...
            param = {"objective": "multi:softprob", "eval_metric": "mlogloss", "num_class": num_classes, "seed": seed}
            ...
            _, val_acc, _ = train_torch_mlp(
                X_train, y_train, X_val, y_val,
                mlp_arch, num_classes, device="cpu", epochs=mlp_epochs,
...
def train_and_save_all_models_with_cv(
    ...
    for seed in seeds:
        ...
        pipe = Pipeline([('scaler', StandardScaler()), ('clf', LogisticRegression(max_iter=1000, multi_class='ovr'))])
        ...
        if lgb is not None:
            ...
            lgbm = lgb.LGBMClassifier(objective='multiclass', num_class=num_classes, random_state=seed)
            grid = GridSearchCV(lgbm, param_grid, cv=cv, scoring='accuracy', n_jobs=-1)
            grid.fit(X_train, y_train)
        ...
        if torch is not None:
            ...
            for epochs in mlp_epoch_grid:
                for lr in mlp_lr_grid:
                    ...
                    for train_idx, val_idx in cv.split(X_train, y_train):
                        ...
                        _, val_acc, _ = train_torch_mlp(
                            X_tr, y_tr, X_val, y_val, mlp_arch, num_classes,
            ...
            print(f"Best MLP params: {best_params}, CV Accuracy={best_acc:.4f}")

            # Train final MLP on full training set with best hyperparameters
            model, _, _ = train_torch_mlp(
                X_train, y_train, X_train, y_train, mlp_arch, num_classes,
...
def evaluate_checkpoints(checkpoint_root: str, X_test: np.ndarray, y_test: np.ndarray, arch: tuple, num_classes: int) -> pd.DataFrame:
    ...
    for seed_dir in sorted(root.glob('seed_*')):
        ...
        if tabpfn_path.exists() and TabPFNClassifier is not None:
            ...
            auc = roc_auc_score(y_test, probs, multi_class='ovr')
            rows.append({'model': 'TabPFN', 'seed': seed, 'roc_auc': auc})
        ...
        if (mlp_final.exists() or (ckpt_subdir.exists() and any(ckpt_subdir.glob('mlp_epoch*.pt')))) and torch is not None:
            ...
            if mlp_final.exists():
                ...
                model = TorchMLP(input_dim=X_test.shape[1], n_layers=arch[0], width=arch[1], output_dim=num_classes)
...
if __name__ == '__main__':
    ...
    # Evaluate
    df_results = evaluate_checkpoints(checkpoint_dir, X_test, y_test, mlp_arch, num_classes)
